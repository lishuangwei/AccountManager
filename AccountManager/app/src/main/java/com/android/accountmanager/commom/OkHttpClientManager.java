...
//	private CacheControl _cacheControl = null;
	 ...
	 * @throws IOException
	 */
	public static String getSync(Context context, String url, Param... params) throws IOException {
		return getHttpInstance(context).getSyncServer(url, params);
	}
...
//		Log.e("网络请求", "post请求结果：" + response.isSuccessful() + " | 缓存数据：" + response.cacheResponse() + " | 网络数据：" + response.networkResponse());
	...
	private String getSyncServer(String url, Param... params) throws IOException {
...
//		Log.e("网络请求", "get请求结果：" + execute.isSuccessful() + " | 缓存数据：" + execute.cacheResponse() + " | 网络数据：" + execute.networkResponse());
	...
	private Request buildGetRequest(String url, Param[] params) {
		...
		for (Param param : params) {
			pstr.append(join + param.toString());
			join = '&';
		}
		return new Request.Builder().url(url + pstr.toString()).build();
	}
